...
sim <- function(x){
  ...
  passage_sample <- sample(1:8, 1, replace = TRUE) 
  dam_passed <- hudson_modern$dam_name[passage_sample]
  passage[1:passage_sample] <- 0.75
  ...
  res$passage_sample <- passage_sample
  res$dam_passed <- dam_passed
...
save(result, file = "results/sim_result_quality_sav_passage.rda")
...
resdf$dam <- trimws(substr(resdf$dam_passed, 1, 9))
